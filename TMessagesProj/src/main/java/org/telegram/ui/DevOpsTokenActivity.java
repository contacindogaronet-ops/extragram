...
public class DevOpsTokenActivity extends BaseFragment {

    private ListAdapter listAdapter;
    private RecyclerListView listView;
    private final ArrayList<ItemInner> items = new ArrayList<>();

    private int setTokenRow;
    private int revokeTokenRow;
    ...
    private void updateRows() {
        items.clear();
        int row = 0;
        items.add(ItemInner.asHeader("CREDENTIALS MANAGEMENT"));
        setTokenRow = row++;
        items.add(ItemInner.asSetting("Configure Custom API Token"));
        
        revokeTokenRow = row++;
        items.add(ItemInner.asSetting("Revoke & Clear Stored Keys"));
    ...
    public View createView(Context context) {
        ...
        actionBar.setTitle("Manage API & Secret Tokens");
